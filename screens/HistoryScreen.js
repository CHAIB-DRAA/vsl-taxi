...
import * as FileSystem from 'expo-file-system'; // Pour le CSV
...
export default function HistoryScreen({ navigation }) {
  ...
  const saveEdit = async () => {
    try {
      ...
      if (editData.endTime && editData.endTime.includes(':')) {
        ...
        const newEndTime = baseDate.hour(parseInt(hours)).minute(parseInt(minutes)).toISOString();
  ...
  // ============================================================
  // 2. EXPORT CSV (NOUVEAU)
  // ============================================================
  const generateCSV = async () => {
    if (filteredRides.length === 0) return Alert.alert("Info", "Rien à exporter.");
    try {
      let csvContent = "Date;Heure;Patient;Départ;Arrivée;Type;Distance (km);Péages (€);Statut Facturation\n";
      filteredRides.forEach(ride => {
        const date = moment(ride.date).format('DD/MM/YYYY');
        const time = moment(ride.startTime || ride.date).format('HH:mm');
        const dist = (ride.realDistance || ride.distance || 0).toString().replace('.', ',');
        const tolls = (ride.tolls || 0).toString().replace('.', ',');
        csvContent += `${date};${time};${ride.patientName};${ride.startLocation};${ride.endLocation};${ride.type};${dist};${tolls};${ride.statuFacturation || 'Non facturé'}\n`;
      });

      const fileName = `Export_${currentDate.format('MM_YYYY')}.csv`;
  ...
  const pickFromGallery = async (docType) => {
    ...
    } catch (error) { Alert.alert("Erreur", "Galerie inaccessible"); }
  };
                ...
                <View style={styles.timeRow}>
                  ...
                  <View style={{ alignItems: 'center' }}>
                    {isEditing ? (
                      ...
                      <Text style={styles.timeValue}>
                        {selectedRide.endTime ? moment(selectedRide.endTime).format('HH:mm') : (selectedRide.arrivalDate ? moment(selectedRide.arrivalDate).format('HH:mm') : '--:--')}
                      </Text>
                    )}
